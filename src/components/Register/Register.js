    ...
    handleProductSubmit = async (e) => {
        ...
        try {
            ...
            console.log(parsedResponse)
                ...
                } else if (parsedResponse.errmsg === `E11000 duplicate key error collection: heroku_9hv1klh9.products index: email_1 dup key: { : "${email}" }`) {
                    this.setState({
                        errorMsgProduct: "This email has already registered. Please enter a different email."
                    })
                } else if (parsedResponse.errmsg === `E11000 duplicate key error collection: heroku_9hv1klh9.products index: name_1 dup key: { : "${name}" }`) {
                    this.setState({
    ...
    handleClientSubmit = async (e) => {
        ...
        try {
            ...
            console.log(parsedResponse)
                ...
                } else if (parsedResponse.error.errmsg === `E11000 duplicate key error collection: heroku_9hv1klh9.clients index: email_1 dup key: { : "${email}" }`) {
                    this.setState({
                        errorMsgClient: "This email has already registered. Please enter a different email."
                    })
                } else if (parsedResponse.error.errmsg === `E11000 duplicate key error collection: heroku_9hv1klh9.clients index: name_1 dup key: { : "${name}" }`) {
                    this.setState({
